...
SELECT year, make, model,
	CONCAT(hwy, ' highway, ', cty, ' city.') AS miles_per_gallon
FROM vehicles;

/* 10. Write a query that returns the id, make, model, and year for all records that have 
NULL for either the cyl or displ fields. */
...
SELECT *
FROM vehicles
WHERE drive = 'Rear-Wheel Drive' AND fuel = 'Diesel'
...
WHERE (make = 'Ford' OR make = 'Chevrolet')
	AND (LOWER(class) = 'compact cars' OR LOWER(class) = 'two seaters');
--ANS: 612
...
/* Write a query that returns all the records of vehicles since the year 2000 whose model name starts with a capital X. 
...
WHERE year >= 2000
	AND model LIKE 'x%'
ORDER BY make;
-- The LIKE operator is case sensitive. lowercase 'x' returns different records from uppercase 'X'
...
/* 16. Write a query that returns the count of all records before the year 2000 that got more than 20 mpg hwy and had greater
than 3 liters displacement (“displ” field). Provide the query as well as the answer. */
